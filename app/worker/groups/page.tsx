...
import { getWorkerByUserId, getWorkerGroupDetails } from '@/lib/db/worker-actions'
import WorkerGroupsClient from './WorkerGroupsClient'
...
export default async function WorkerGroupsPage() {
  ...
  // Get actual group details and team members
  const groupDetails = await getWorkerGroupDetails(workerData.id)

  return <WorkerGroupsClient worker={worker} groupDetails={groupDetails} currentUserId={user.id} />
}
