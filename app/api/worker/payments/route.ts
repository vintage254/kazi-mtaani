...
import { getWorkerPaymentHistory } from '@/lib/db/worker-actions'
...
export async function GET() {
  try {
    ...
    const paymentHistory = await getWorkerPaymentHistory(workerData.id, 10)
    ...
    return NextResponse.json(
      { error: 'Failed to fetch worker payment history' },
      { status: 500 }
    )
  }
}
